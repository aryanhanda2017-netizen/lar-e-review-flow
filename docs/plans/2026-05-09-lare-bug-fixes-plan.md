...
- Frontend: `/tmp/lare-review/frontend/` (clone of `github.com/aryanhanda2017-netizen/lar-e-front`)
...
| `pages/LoginPage.tsx` + `contexts/AuthContext.tsx` | Only persist token after /api/auth/me succeeds |
...
- [ ] **Step 3: Add pagination UI below the leads list**

After the leads table/list JSX, add:
```tsx
<div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 16 }}>
  <span className="t-label t-muted">
    Showing {Math.min(leadsPage * PAGE_SIZE, totalLeads)} of {totalLeads} leads
  </span>
  {hasMore && (
    <button
